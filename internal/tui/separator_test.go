...
// Neither pane may be collapsed below what it can render — each would render its
// floor anyway and the surplus clips the status bar. Dragging to either extreme
// stops at the other pane's minimum.
func TestPreviewHeightForSeparatorY_ClampsBothEnds(t *testing.T) {
	...
	require.Equal(t, common.MinPreviewHeight, previewHeightForSeparatorY(1000, available),
		"dragging past the bottom still leaves the preview its minimum height")
